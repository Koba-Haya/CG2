...
------------------------*/
  ...
  for (UINT i = 0; dxgiFactory->EnumAdapterByGpuPreference(
    ...
    if (!(adapterDesc.Flags & DXGI_ADAPTER_FLAG3_SOFTWARE)) {
      // 採用したアダプタの情報をログに出力。wstringのほう
      Log(ConvertString(
          std::format(L"Use Adapter:{}\n",
                      adapterDesc.Description))); // ConvertString関数を追加した
      break;
    }
  ...
  rtvDesc.Format =
      DXGI_FORMAT_R8G8B8A8_UNORM_SRGB; // 出力結果をSRGBの変換して書き込む
  ...
  // バイナリをもとに生成
  ID3D12RootSignature *rootSinature = nullptr;
  hr = device->CreateRootSignature(0, signatureBlob->GetBufferPointer(),
                                   signatureBlob->GetBufferSize(),
                                   IID_PPV_ARGS(&rootSinature));
  assert(SUCCEEDED(hr));
  ...
  graphicsPipelineStateDesc.pRootSignature = rootSinature; // RootSignature
  graphicsPipelineStateDesc.InputLayout = inputLayoutDesc; // InpuyLayout
  ...
  *materialData = Vector4(1.0f, 0.0f, 0.0f, 1.0f);
  ...
  while (msg.message != WM_QUIT) {
    ...
    if (PeekMessage(&msg, NULL, 0, 0, PM_REMOVE)) {
      ...
      commandList->SetGraphicsRootSignature(rootSinature);
