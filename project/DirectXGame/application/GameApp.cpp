  ...
  // ===== 平行光（InitResources_ で作った CB を使う） =====
  {
    ...
    if (showEmitterGizmo_) {
      if (emitterShape_ == EmitterShape::Box) {
        ...
        // 球: Extent の max を半径にする
        float radius =
            std::max(particleSpawnExtent_.x,
                     std::max(particleSpawnExtent_.y, particleSpawnExtent_.z));
        if (radius < 0.001f)
          radius = 0.001f;
...
void GameApp::RespawnParticle_(Particle &p) {
  ...
  if (emitterShape_ == EmitterShape::Box) {
    ...
    pos.x += local.x * radius;
    pos.y += local.y * radius;
    pos.z += local.z * radius;
  }
